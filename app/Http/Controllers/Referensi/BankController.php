...
use App\Http\Requests\Referensi\StoreBank;
...
class BankController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $fields =
            ['id', 'kode', 'nama', 'singkatan', 'logo', 'ishide', 'reorder'];
        $data = Bank::where(function ($query) use ($request, $fields) {
        ...
        $data = $data->withCount('rekeningBank')->paginate($request->itemsPerPage);
    ...
    public function updateOrCreate(StoreBank $request)
    {
        $request->validated();

        $val = Bank::updateOrCreate($request->only('id'), $request->all());
    ...
    public function destroys(Request $request)
    {
        ...
        foreach ($request->item as $key => $value) {
            $data = Bank::where('id', $value['id'])->first();

            if (!$data) {
                $status = false;
                $fail++;
            } else {
                $val = Bank::destroy($value['id']);
                $status = true;
                $success++;
            }
        }
    ...
    {
        $hide = Bank::where('id', $request->id)->pluck('ishide')->first();
        $val = Bank::where('id', $request->id)->update(['ishide' => $hide == 1 ? 0 : 1]);
    ...
    {
        $val = Bank::where('id', $request->id)->update(['reorder' => $request->reorder]);
